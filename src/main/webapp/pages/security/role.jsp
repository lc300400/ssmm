...
<body>
	<nav class="breadcrumb">
		<i class="Hui-iconfont">&#xe67f;</i>
		 ...
		 角色管理 <a class="btn radius r defaultColor" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" >
	...
	<div class="page-container">
		...
		<div class="text-c">
			<form class="form-inline" id="searchForm">
				<input type="text" class="input-text" name="name" style="width:200px" placeholder="角色名称">
...
var dataTable;
/*var visibleOperate = false;
<sec:authorize access="hasRole('A_Authority')">
	visibleOperate = true;
</sec:authorize> */
$(function(){
	/* 选中菜单 */
	/* $("#menu-security > dt").click();
	$("#menu-security-role").addClass("current");
	 */
	/* 表格初始化 */
...
function addFun(){
	layer.open({
		...
		maxmin: true,
		title: "新增角色",
		content: "${ctx}/security/role/savePage"
	});
}
...
function editFun(id){
	layer.open({
		...
		maxmin: true,
		title: "编辑角色",
		content: "${ctx}/security/role/savePage?id="+id
	});
}
...
function delFun(id){
	layer.confirm("确认要删除吗？", function(index){
		$.ajax({
			url: "${ctx}/security/role/delete?id="+id,
			type: "post",
